...
int main(int argc, char *argv[]) {
    ...
    if (argc - optind == 0) {
        ...
        if (ftw(P_dir, scan_dir_for_dynamic_lib_options_if_user_provide_no_dir_for_scan_via_dynamic_lib, 10) ==
            -1) // just printing info for all available plugins
        ...
        if (ftw(P_dir, scan_dir_via_dynamic_lib_or_libs_for_matches, 10) == -1) // scan Plugins dir for plugins and
            print_error_message(
                    "ftw");                                                    //  scan dir to scan via every plugin
        // in Plugins dir
        apply_logic(dir_to_scan, A_flag,
                    N_flag); // filter massive of valid files( plugin returned 0 for it) via bool logic
